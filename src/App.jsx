...
// 1. ADD STATE FOR MOBILE MENU & RESPONSIVE DETECTION:
const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
const [isMobile, setIsMobile] = useState(false);
...
// 2. MOBILE HAMBURGER HEADER (replace current layout):
{isMobile && (
  <div style={{
    display: 'flex',
    alignItems: 'center',
    ...
    <button
      ...
      style={{
        background: mobileMenuOpen ? accent + '22' : 'transparent',
        border: `1px solid ${mobileMenuOpen ? accent : C.border}`,
        borderRadius: 6,
        padding: '8px 12px',
        color: mobileMenuOpen ? accent : C.text,
        cursor: 'pointer',
        fontSize: 18,
        fontWeight: 700,
        width: 40,
...
// 3. CONDITIONAL SIDEBAR (show/hide on mobile):
{(!isMobile || mobileMenuOpen) && (
  <div style={{
...
// 4. OVERLAY FOR MOBILE (close menu on tap):
{isMobile && mobileMenuOpen && (
  <div
